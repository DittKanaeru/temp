module.exports = class NeoxrApi {
  ...
  async spotify(str) {
    let json = str.startsWith("https")
      ? await Func.fetchJson(
          this.baseUrl + "/spotify?url=" + str + "&apikey=" + this.apiKey,
        )
      : await Func.fetchJson(
          this.baseUrl + "/spotify-search?q=" + str + "&apikey=" + this.apiKey,
        );
    if (json?.creator) {
      json.creator = process.env.DEVELOPER || "© Dixie Developers";
    }
    return json;
  }
