...
module.exports = {
  ...
  uploadToMaelyn: async (buffer) => {
    try {
      // Gunakan fromBuffer dari modul file-type yang sudah diimport
      ...
      const formData = new FormData();
      const randomBytesStr = crypto.randomBytes(5).toString("hex");
      formData.append("file", buffer, `${randomBytesStr}.${ext || mime.split('/')[1]}`);
