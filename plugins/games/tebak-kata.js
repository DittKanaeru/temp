module.exports = {
  help: ["tebakkata"],
  tags: ["games"],
  command: ["tebakkata", "kataclue", "kataskip"],
  run: async (m, { conn, env, command, usedPrefix }) => {
    ...
    try {
      let json = await Func.fetchJson(
        "https://raw.githubusercontent.com/Dwi-Merajah/Database-Public/main/games/tebakkata.json",
      );
      ...
      let teks = `🌸  *T E B A K - K A T A*  🌸 \n\n`;
      teks += `${soal}\n\n`;
      teks += `Waktu: [ *${(env.timeout / 1000).toFixed(0)} detik* ]\n`;
      teks += `Reply pesan ini untuk menjawab.\n`;
      teks += `Ketik *${usedPrefix}kataclue* untuk bantuan dan *${usedPrefix}kataskip* untuk melewati soal.`;
  ...
  before: async (m, { conn, env, users, prefixes }) => {
    ...
    if (!conn.tebakkata[m.chat] && /kataclue/i.test(m.quoted.text))
      return conn.reply(
        m.chat,
        Func.texted(
          "bold",
          `📮 Soal tersebut telah berakhir, silahkan kirim _${prefixes[0]}tebakkata_ untuk mendapatkan soal baru.`,
        ),
        m,
      );
    if (m.quoted && /kataclue/i.test(m.quoted.text)) {
      let gameData = conn.tebakkata[m.chat];
      ...
      if (userAnswer === correctAnswer) {
        ...
        teks += `send .tebakkata to play again.`;
        m.reply(teks).then(() => {
          users.yuan += yuan;
          users.exp += exp;
