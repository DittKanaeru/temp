module.exports = {
  help: ["tesotak"],
  tags: ["games"],
  command: ["tesotak", "otakclue", "otakskip"],
  run: async (m, { conn, env, command, usedPrefix }) => {
    conn.tesotak = conn.tesotak || {};
    if (command === "otakclue") {
      if (!conn.tesotak[m.chat])
        return conn.reply(m.chat, "❌ Tidak ada soal aktif!", m);
      let ans = conn.tesotak[m.chat][1]?.jawaban?.trim();
      if (!ans) return conn.reply(m.chat, "⚠️ Clue tidak tersedia!", m);
      let clue = ans.replace(/[AIUEOaiueo]/g, "_");
      return conn.reply(m.chat, `💡 *Clue:* ${clue}`, conn.tesotak[m.chat][0]);
    }
    if (command === "otakskip") {
      if (!conn.tesotak[m.chat])
        return conn.reply(m.chat, "❌ Tidak ada soal yang perlu di-skip!", m);
      let ans = conn.tesotak[m.chat][1]?.jawaban;
      if (!ans) return conn.reply(m.chat, "⚠️ Soal tidak valid!", m);
      conn.reply(
        m.chat,
        `⚠️ *Soal di-skip!*\n\n📖 *Jawaban:* ${ans}`,
        conn.tesotak[m.chat][0],
      );
      delete conn.tesotak[m.chat];
      return;
    }
    if (conn.tesotak[m.chat])
      return conn.reply(
    ...
    try {
      let json = await Func.fetchJson(
        "https://raw.githubusercontent.com/Dwi-Merajah/Database-Public/main/games/tesotak.json",
      );
      ...
      let teks = `🌸  *T E S - O T A K*  🌸 \n\n`;
      teks += `${soal}\n\n`;
      teks += `Waktu: [ *${(env.timeout / 1000).toFixed(0)} detik* ]\n`;
      teks += `Reply pesan ini untuk menjawab.\n`;
      teks += `Ketik *${usedPrefix}otakclue* untuk bantuan dan *${usedPrefix}otakskip* untuk melewati soal.`;
      conn.tesotak[m.chat] = [
  ...
  before: async (m, { conn, env, users, prefixes }) => {
    ...
    if (!conn.tesotak[m.chat] && /otakclue/i.test(m.quoted.text))
      return conn.reply(
        m.chat,
        Func.texted(
          "bold",
          `📮 Soal tersebut telah berakhir, silahkan kirim _${prefixes[0]}tesotak_ untuk mendapatkan soal baru.`,
        ),
        m,
      );
    if (m.quoted && /otakclue/i.test(m.quoted.text)) {
      let gameData = conn.tesotak[m.chat];
      ...
      if (userAnswer === correctAnswer) {
        ...
        teks += `send .tesotak to play again.`;
        m.reply(teks).then(() => {
          users.yuan += yuan;
          users.exp += exp;
