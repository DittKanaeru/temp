module.exports = {
  ...
  run: async (m, { conn, env, command, usedPrefix }) => {
    ...
    try {
      let json = await Func.fetchJson(
        "https://raw.githubusercontent.com/Dwi-Merajah/Database-Public/main/games/susunkata.json",
      );
      if (!json || json.length === 0) return m.reply("Data kosong!");
      let { soal, jawaban, tipe } = Func.random(json);
      if (!soal || !jawaban || !tipe) return m.reply("Gagal memuat soal!");
      let teks = `🌸  *S U S U N K A T A*  🌸 \n\n`;
      teks += `${tipe} : ${soal}\n\n`;
      teks += `Waktu: [ *${(env.timeout / 1000).toFixed(0)} detik* ]\n`;
      teks += `Reply pesan ini untuk menjawab.\n`;
      teks += `Ketik *${usedPrefix}susunclue* untuk bantuan dan *${usedPrefix}susunskip* untuk melewati soal.`;
  ...
  before: async (m, { conn, env, users, prefixes }) => {
    ...
    if (!conn.susunkata[m.chat] && /susunclue/i.test(m.quoted.text))
      return conn.reply(
        m.chat,
        Func.texted(
          "bold",
          `📮 Soal tersebut telah berakhir, silahkan kirim _${prefixes[0]}susunkata_ untuk mendapatkan soal baru.`,
        ),
        m,
      );
    if (m.quoted && /susunclue/i.test(m.quoted.text)) {
      let gameData = conn.susunkata[m.chat];
      ...
      if (userAnswer === correctAnswer) {
        ...
        teks += `send .susunkata to play again.`;
        m.reply(teks).then(() => {
          users.yuan += yuan;
          users.exp += exp;
